...
public class NetUtils {
    ...
    public static Utils.Task<Boolean> isAvailableByPingAsync(final String ip,
                                                             @NonNull final Utils.Callback<Boolean>
                                                                     callback) {
        return Utils.doAsync(new Utils.Task<Boolean>(callback) {
            @RequiresPermission(INTERNET)
            @Override
    ...
    public static boolean isAvailableByPing(final String ip) {
        ...
        ShellUtils.CommandResult result = ShellUtils
                .execCmd(String.format("ping -c 1 %s", realIp), false);
     ...
     * <p>Must hold {@code <uses-permission
     * android:name="android.permission.ACCESS_WIFI_STATE" />}</p>
     *
     * @return {@code true}: enabled<br>{@code false}: disabled
     */
     ...
     */
    @RequiresPermission(allOf = {ACCESS_WIFI_STATE, INTERNET})
    public static Utils.Task<Boolean> isWifiAvailableAsync(@NonNull final
                                                           Utils.Callback<Boolean> callback) {
        return Utils.doAsync(new Utils.Task<Boolean>(callback) {
            @RequiresPermission(allOf = {ACCESS_WIFI_STATE, INTERNET})
            @Override
            public Boolean doInBackground() {
    ...
    public static Utils.Task<String> getIPAddressAsync(final boolean useIPv4,
                                                       @NonNull final Utils.Callback<String>
                                                               callback) {
